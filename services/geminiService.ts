...
// SMART KEY RETRIEVAL: Checks multiple naming conventions to ensure Vercel build picks it up
const API_KEY = 
  process.env.API_KEY || 
...
export const generateTextResponse = async (
  ...
  if (!API_KEY) throw new Error("API Key not found. Please configure API_KEY in Vercel settings.");
...
export const generateImage = async (prompt: string) => {
  ...
  try {
    ...
    // Note: If this model is not enabled on your key, this will fail. 
    // Fallback to gemini-pro-vision if needed for describing, but for generation imagen-3.0 is standard target.
    ...
    const data = await response.json();
    // Response has 'predictions' array with 'bytesBase64Encoded'
